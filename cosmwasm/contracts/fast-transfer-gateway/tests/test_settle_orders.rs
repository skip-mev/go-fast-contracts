...
fn test_settle_orders() {
    ...
    let solver_hex = HexBinary::from(left_pad_bytes(bech32_decode(solver.as_str()).unwrap(), 32));
    ...
    let execute_msg = ExecuteMsg::Handle(HandleMsg {
        ...
        body: settle_orders_message.encode(),
    });
    ...
    assert_eq!(
        ...
        SubMsg {
            id: 0,
            msg: BankMsg::Send {
                to_address: solver.into(),
                amount: vec![coin(
                    order_a.amount_in.u128() + order_b.amount_in.u128(),
                    "uusdc"
                )],
            }
            .into(),
...
#[test]
fn test_settle_orders_ignores_settled_orders_but_does_not_fail() {
    let (mut deps, env) = default_instantiate();
...
#[test]
fn test_settle_orders_fails_on_unknown_order() {
    let (mut deps, env) = default_instantiate();
...
#[test]
fn test_settle_orders_fails_if_orders_destination_domain_is_not_domain_message_originated_from() {
    let (mut deps, env) = default_instantiate();
